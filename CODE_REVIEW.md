...
- 12.1 新 UI 已於 2026-08-12 完成靜態覆蓋：SocialUI tabs／content／card pools 與 side windows、Chat Config Additional Colors、Housing Collection／Initiatives／獨立 HousingBlueprint、CooldownViewer Group Buff／alert editors／drag preview、PVP Training Ground Arena，以及 GuildControl Discord linked／unlinked。新增 skin 只讀 frame／region，不解析 Social search、Housing share code、Discord 或 aura payload；正式服狀態切換、pool reuse、secure drag 與 LoD 兩種時序仍待驗證。
